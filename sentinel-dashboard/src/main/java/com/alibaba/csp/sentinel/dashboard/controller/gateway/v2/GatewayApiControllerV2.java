 ...
 */
@RestController
@RequestMapping(value = "/v2/gateway")
public class GatewayApiControllerV2 {
    ...
    private void publishApis(String app) throws Exception{
        ...
        List<ApiDefinition> authorityRules = rules.stream().map(ApiDefinitionEntity::toApiDefinition).collect(Collectors.toList());
        rulePublisher.publish(app, ApiDefinitionEntity.GATEWAY_API_PREFIX, authorityRules);
    }

    @GetMapping("/apis")
    @AuthAction(AuthService.PrivilegeType.READ_RULE)
    public Result<List<ApiDefinitionEntity>> queryApis(@RequestParam String app) {
        ...
        try {
            ...
            List<ApiDefinition> apiDefinitions = ruleProvider.getRules(app, ApiDefinitionEntity.GATEWAY_API_PREFIX, ApiDefinition.class);
            MachineInfo machine = appManagement.getFirstMachine(app);
            if(apiDefinitions !=null && !apiDefinitions.isEmpty() && machine.isHealthy()) {
                apis = apiDefinitions.stream().map(v -> ApiDefinitionEntity.fromApiDefinition(machine.getApp(), machine.getIp(), machine.getPort(), v)).collect(Collectors.toList());
                if (!apis.isEmpty()) {
    ...
    @PutMapping("/api/{id}")
    @AuthAction(AuthService.PrivilegeType.WRITE_RULE)
    public Result<ApiDefinitionEntity> updateApi(@PathVariable("id") Long id, @RequestBody  UpdateApiReqVo reqVo) {
        String app = reqVo.getApp();
        if (StringUtil.isBlank(app)) {
            return Result.ofFail(-1, "app can't be null or empty");
        }
