...
public class GatewayFlowRuleControllerV2 {
    ...
    private void publishRules(String app) throws Exception {
        ...
        List<GatewayFlowRule> authorityRules = rules.stream().map(GatewayFlowRuleEntity::toGatewayFlowRule).collect(Collectors.toList());
        rulePublisher.publish(app, GatewayFlowRuleEntity.GATEWAY_FLOW_RULE_PREFIX, authorityRules);
    }

    @GetMapping("/rules")
    @AuthAction(AuthService.PrivilegeType.READ_RULE)
    public Result<List<GatewayFlowRuleEntity>> queryFlowRules(@RequestParam String app) {
        ...
        try {
            ...
            List<GatewayFlowRule> gatewayFlowRules = ruleProvider.getRules(app, GatewayFlowRuleEntity.GATEWAY_FLOW_RULE_PREFIX, GatewayFlowRule.class);
            MachineInfo machine = appManagement.getFirstMachine(app);
            if(gatewayFlowRules !=null && !gatewayFlowRules.isEmpty() && machine.isHealthy()) {
                rules = gatewayFlowRules.stream().map(v -> GatewayFlowRuleEntity.fromGatewayFlowRule(machine.getApp(), machine.getIp(), machine.getPort(), v)).collect(Collectors.toList());
                if (!rules.isEmpty()) {
    ...
    @PutMapping("/rule/{id}")
    @AuthAction(AuthService.PrivilegeType.WRITE_RULE)
    public Result<GatewayFlowRuleEntity> updateRule(@PathVariable("id") Long id, @RequestBody UpdateFlowRuleReqVo reqVo) {
        String app = reqVo.getApp();
        if (StringUtil.isBlank(app)) {
            return Result.ofFail(-1, "app can't be null or empty");
        }
